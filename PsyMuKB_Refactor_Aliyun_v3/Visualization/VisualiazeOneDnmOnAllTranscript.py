					...
					if item_item.get("Position") == target_dict.get("position"):
						if item_item['Exonic Func'] != 'null':
							if isinstance(item_item['Exonic Func'], str):
								s = item_item['Position'] + ',' + item_item['Variant'] + ',' + item_item[
									'Exonic Func']
		...
		for item in all_frag:
			...
			for kk, item_item in enumerate(DNM_list):
				if item_item >= item[self.Start] and item_item <= item[self.End]:
					...
					temp.append(temp1)
			# if kk == len(DNM_list)-1:
			# 	if item_item >= item[self.End]:
			# 		num[i] += 1
			# 		temp1 = []
			# 		temp1.append(item_item)
			# 		temp1.append(DNM_tips[kk])
...
class PlotRun(object):
	...
	def run(self, id):
		a = DataQuery("Gene", self.mutation)
...
def main(ID):
	mutaion = 'gene=4282,location=coding-dnm,position=24236689,chr=22'
