	...
	def get_ep_data(self):
		...
		if self.gene_data != None:
				...
				for item_item in self.gene_data['DNM']:
					if 'enhancerid' in item_item.keys():
						if 'e_start' in item_item.keys() and 'e_end' in item_item.keys():
							if item_item['e_start'] not in ['-', None, 'NA'] and item_item['e_end'] not in ['-', None, 'NA']:
								tips = item_item['enhancerid']+','+'['+item_item['e_start'].replace("'","")+','+item_item['e_end'].replace("'","")+']'+',targetgene'
								temp_enhancer.append('Enhancer')
					...
					if 'promoterid' in item_item.keys():
						if 'p_start' in item_item.keys() and 'p_end' in item_item.keys():
							if item_item['p_start'] not in ['-', None, 'NA'] and item_item['p_end'] not in ['-', None, 'NA']:
								tips = item_item['promoterid']+','+'['+item_item['p_start'].replace("'","")+','+item_item['p_end'].replace("'","")+']'+',targetgene'
			...
			if temp_enhancer !=[] and temp_enhancer_frag!=[] and temp_enhancer_tips!=[]:
				temp_enhancer, temp_enhancer_frag, temp_enhancer_tips = self.drop_duplication(temp_enhancer, temp_enhancer_frag, temp_enhancer_tips)
			...
			if temp_promoter != [] and temp_promoter_frag != [] and temp_promoter_tips != []:
				temp_promoter, temp_promoter_frag, temp_promoter_tips = self.drop_duplication(temp_promoter, temp_promoter_frag, temp_promoter_tips)
		...
		if strand == '+':
			if type(row['utr5_exon_region']) == str and row['utr5_exon_region'] != 'NA':
				if type(row['utr3_exon_region']) == str and row['utr3_exon_region'] != 'NA':
					...
					if type(row['coding_exon_region']) == str and row['coding_exon_region'] != 'NA':
						ei = [self.str_to_int(row['utr5_exon_region'])[1], self.GetExonRegion(row['coding_exon_region'])[1]]
					else:
						ei = []
			else:
				if type(row['coding_exon_region']) == str and row['coding_exon_region'] != 'NA':
					if type(row['utr3_exon_region']) == str and row['utr3_exon_region'] != 'NA':
						ei = [self.GetExonRegion(row['coding_exon_region'])[0], self.str_to_int(row['utr3_exon_region'])[0]]
					else:
			...
			if type(row['utr3_exon_region']) == str and row['utr3_exon_region'] != 'NA':
				if type(row['utr5_exon_region']) == str and row['utr5_exon_region'] != 'NA':
					...
					if type(row['coding_exon_region']) == str and row['coding_exon_region'] != 'NA':
						ei = [self.str_to_int(row['utr3_exon_region'])[1], self.GetExonRegion(row['coding_exon_region'])[1]]
					else:
						ei = []
			else:
				if type(row['coding_exon_region']) == str and row['coding_exon_region'] != 'NA':
					if type(row['utr5_exon_region']) == str and row['utr5_exon_region'] != 'NA':
						ei = [self.GetExonRegion(row['coding_exon_region'])[0], self.str_to_int(row['utr5_exon_region'])[0]]
					else:
						ei = self.GetExonRegion(row['coding_exon_region'])
				else:
					ei = []
		...
		if self.plot_data.promoter_plot_data!=None:
			for item in self.plot_data.promoter_plot_data:
				trace_item = Scatter(
					...
					x=[item[0][0], item[1][0]],
					y=[item[0][1], item[1][1]],
					text=str(item[0][2]).replace(',robust','').replace(',permissive','').replace(',targetgene','').replace('local',''),
					line=dict(
