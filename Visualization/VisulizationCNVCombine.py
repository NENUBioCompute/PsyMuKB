...
class Cnv_gene:
        ...
        for i in range(len(nums) - 1):  # 这个循环负责设置冒泡排序进行的次数
            for j in range(len(nums) - i - 1):  # ｊ为列表下标
                if self.get_index(chr_list, nums[j]) != None and self.get_index(chr_list, nums[j + 1]) != None:
                    ...
                    pass
        # if self.get_index(chr_list, nums[j])==None:
        # 	if nums[j] not in t:
        # 		t.append(nums[j])
        # if self.get_index(chr_list, nums[j + 1])==None:
        # 	if nums[j + 1] not in t:
        # 		t.append(nums[j + 1])
        # print(t)
        return nums
    ...
    def cnv_gene(self, data):
        ...
        annotations.append(dict(xref='paper', x=0.02, y=[number_cnv],
                                xanchor='right', yanchor='middle',
                                text="<b>Phenotype: Location:VariantType</b>",
                                font=dict(family='Arial',
                                          size=10,
                                          color=colors, ),
                                # tickangle=-40,
...
class Main:
    ...
    def split_cnv_id(self, cnv_id):
        chr = cnv_id.split(':')[0]
        if '-' in cnv_id.split(':')[1]:
            pos_start, pos_end = cnv_id.split(':')[1].split('-')
        elif '¨' in cnv_id.split(':')[1]:
            pos_start, pos_end = cnv_id.split(':')[1].split('¨')
        mut_type = cnv_id.split(':')[-1].lower()
        if not (pos_start.isdigit() and pos_end.isdigit()):
            try:
                pos_start = pos_start.replace('C', '')
                pos_end = pos_end.replace('C', '')
            except:
                pos_start = ''
                pos_end = ''
        return (chr, mut_type, pos_start, pos_end)
    ...
    def run(self, id):
        ...
        if data != None:
            if 'CNV' in data.keys():
                case_f = 0
                control_f = 0
                # disorder_and_type_list = []
                disorder_and_type_dict = {}
                for item in data['CNV']:
                    if self.split_cnv_id(item.get('CNV_ID'))[0] + '_' + item.get('Disorder') + '_' + \
                            self.split_cnv_id(item.get('CNV_ID'))[1] not in disorder_and_type_dict.keys():
                        disorder_and_type_dict[
                            self.split_cnv_id(item.get('CNV_ID'))[0] + '_' + item.get('Disorder') + '_' +
                            self.split_cnv_id(item.get('CNV_ID'))[1]] = []
                    if item.get('Disorder') == 'Control':
                        control_f = 1
                ...
                for item in data['CNV']:
                    disorder_and_type_dict[self.split_cnv_id(item.get('CNV_ID'))[0] + '_' + item.get('Disorder') + '_' +
                                           self.split_cnv_id(item.get('CNV_ID'))[1]].append(item)
                new_data = []
                for item in disorder_and_type_dict.keys():
                    pos_list = []
                    gene_set = []
                    entrez_id_set = []
                    for item_item in disorder_and_type_dict.get(item):
                        if self.split_cnv_id(item_item.get('CNV_ID'))[2].isdigit():
                            pos_list.append(int(self.split_cnv_id(item_item.get('CNV_ID'))[2]))
                        if self.split_cnv_id(item_item.get('CNV_ID'))[3].isdigit():
                            pos_list.append(int(self.split_cnv_id(item_item.get('CNV_ID'))[3]))
                        for item_item_item in item_item.get('genesymbol'):
                            if item_item_item not in gene_set:
                                gene_set.append(item_item_item)
                        for item_item_item in item_item.get('ENTREZ_ID'):
                            if item_item_item not in entrez_id_set:
                                entrez_id_set.append(item_item_item)
                    pos = (min(pos_list), max(pos_list))

                    new_data.append(
                        {'chr': item.split('_')[0], 'Disorder': item.split('_')[1], 'mutation type': item.split('_')[2],
                         'Patient ID': '', 'pmid': '-', 'start': str(pos[0]), 'end': str(pos[1]), 'First author': '-',
                         'of probands': '-', 'Band': '-', 'genesymbol': gene_set, 'ENTREZ_ID': entrez_id_set,
                         'CNV_ID': item.split('_')[0] + ':' + str(pos[0]) + '-' + str(pos[1]) + ':' + item.split('_')[
                             2]})
                res_data = data
                res_data['CNV'] = new_data
                # print(control_f, end='\t,')
                # print(case_f)


                if control_f == 1 and case_f == 0:
                    return dan_cg.cnv_gene(res_data)

                elif control_f == 0 and case_f == 1:
                    return dan_cg.cnv_gene(res_data)

                elif control_f == 1 and case_f == 1:
                    return cg.cnv_gene(id, f, res_data)
        else:
            pass
